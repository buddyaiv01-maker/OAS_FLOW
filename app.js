(() => {
  ...
  function createNode(type, x, y, opts) {
    ...
    nodeData[id] = { type, name: meta.label, sub, desc };
    wireNode(el);
  ...
  function renderConnectionBlock(type, meta, container) {
    ...
    let selectedId = credSelectedId[type];
    if (selectedId === undefined) selectedId = list.length ? list[0].id : "__new__";
    if (selectedId !== "__new__" && !list.some(c => c.id === selectedId)) selectedId = list.length ? list[0].id : "__new__";
    credSelectedId[type] = selectedId;
  ...
  function onConnectionAction(e) {
    ...
    if (action === "toggle-key-visibility") {
      ...
      const currentId = credSelectedId[type];
      const existing = currentId && currentId !== "__new__" ? list.find(c => c.id === currentId) : null;
      if (existing) {
        existing.name = name;
        existing.values = values;
        credSelectedId[type] = existing.id;
      } else {
        credIdCounter += 1;
        const id = "cred-" + Date.now() + "-" + credIdCounter;
        list.push({ id, name, values });
        credSelectedId[type] = id;
      }
      renderConnectionBlock(type, meta, container);
      refreshNodeCheckmarks(type);
      toast(`"${name}" saved`);
      ...
      credSelectedId[type] = credentialStore[type].length ? credentialStore[type][0].id : "__new__";
      renderConnectionBlock(type, meta, container);
      refreshNodeCheckmarks(type);
  ...
  function onConnectionChange(e) {
    ...
    const meta = nodeTypeLibrary[type];
    credSelectedId[type] = e.target.value;
    renderConnectionBlock(type, meta, container);
  }
  ...
  function openNodeModal(id) {
    ...
    renderConnectionBlock(data.type, meta);
  ...
  function closeNodeModal() {
    if (selectedNodeId && nodeData[selectedNodeId]) {
      ...
      const newName = modalTitle.textContent.trim() || data.name;
      data.name = newName;
  ...
  const nodeTemplates = {
    ...
    "wf-donation": {
      // Mirrors a Make.com blueprint: util:SetVariables -> google-email:sendAnEmail
      nodes: [
  ...
  function buildWorkflowCanvas(id) {
    clearCanvas();
    resetHistory();
    const tmpl = nodeTemplates[id];
    if (!tmpl) return; // blank / untemplated workflow — clearCanvas() already shows the empty state
    tmpl.nodes.forEach(n => createNode(n.type, n.x, n.y, { id: n.id, sub: n.sub, desc: n.desc }));
    tmpl.edges.forEach(([from, to]) => createEdgePath(from, to));
    redrawEdges();
    if (tmpl.selectId) selectNode(tmpl.selectId);
  }

  buildWorkflowCanvas("wf-demo");
